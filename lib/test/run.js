...
async function runHttpOffline() {
  ...
  const sampleProviders = [
    { pubkey: 'a'.repeat(64), id: 'p1', publishedAt: new Date(), name: 'Provider A',
      country: 'TZ', direction: 'off-ramp', rail_in: 'lightning', rail_out: 'm-pesa',
    ...
    { pubkey: 'b'.repeat(64), id: 'p2', publishedAt: new Date(), name: 'Provider B',
      country: 'KE', direction: 'on-ramp', rail_in: 'lightning', rail_out: 'bank',
...
async function runHardening() {
  ...
  const baseListing = {
    ...
    rail_in: 'lightning', currency: 'TZS',
  ...
  const legacyEvent = stPub.buildEvent({ ...baseListing, rail_out: 'm-pesa' });
  ...
  const merchantWithRail = stPub.buildEvent({ ...baseListing, id: 's5', service_type: 'merchant-payment', rail_out: 'cash' });
  ...
  try { stPub.buildEvent({ ...baseListing, service_type: 'crypto-laundering', rail_out: 'cash' }); } catch { badTypeThrew = true; }
  ...
  check('parsing an airtime-data listing yields null rail_out (none was set)', airtimeParsed.rail_out === undefined);
