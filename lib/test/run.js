...
const { createProviderHostedTransport, WELL_KNOWN_PATH } = require('../src/transports/provider-hosted/server');
...
function runOffline() {
  ...
  check('HEALTHY_STATUS_VALUES includes active and ok',
    HEALTHY_STATUS_VALUES.includes('active') && HEALTHY_STATUS_VALUES.includes('ok'));
  ...
  check('alt tag present (NIP-31)', lt.alt === 'Lipa Bitcoin service listing');
  ...
  // status_reason (Nostr listing side, mirrors provider-hosted validation)
  const baseListing = {
    id: 'p1-status-test', name: 'Provider A', country: 'TZ', direction: 'off-ramp',
    rails: { in: ['lightning'], out: ['m-pesa'] }, currency: 'TZS'
  };
  let statusReasonThrew = false;
  let statusReasonMsg = '';
  try {
    publisher.buildEvent({ ...baseListing, status: 'paused', status_reason: 'not_a_real_reason' });
  } catch (err) { statusReasonThrew = true; statusReasonMsg = String(err.message || err); }
  check('rejects invalid status_reason value', statusReasonThrew && statusReasonMsg.includes('status_reason'));

  let statusMisuseThrew = false;
  let statusMisuseMsg = '';
  try {
    publisher.buildEvent({ ...baseListing, status: 'active', status_reason: 'maintenance' });
  } catch (err) { statusMisuseThrew = true; statusMisuseMsg = String(err.message || err); }
  check('rejects status_reason when status is not paused', statusMisuseThrew && statusMisuseMsg.includes('only valid when'));

  let pausedOk = true;
  let pausedEvent;
  try {
    pausedEvent = publisher.buildEvent({ ...baseListing, status: 'paused', status_reason: 'out_of_float' });
  } catch { pausedOk = false; }
  const pausedTags = Object.fromEntries((pausedEvent ? pausedEvent.tags : []).map(t => [t[0], t[1]]));
  check('accepts valid status_reason when status is paused', pausedOk);
  check('paused listing emits status_reason tag', pausedTags.status_reason === 'out_of_float');

  let noReasonEvent;
  try {
    noReasonEvent = publisher.buildEvent({ ...baseListing, status: 'active' });
  } catch { /* ignore */ }
  const noReasonTags = Object.fromEntries((noReasonEvent ? noReasonEvent.tags : []).map(t => [t[0], t[1]]));
  check('active listing has no status_reason tag', noReasonTags.status_reason === undefined);
  ...
  check('attestation alt tag present', vt.alt === 'Lipa Bitcoin provider attestation (vouch)');
  ...
  check('revocation alt tag present', rt.alt === 'Lipa Bitcoin provider trust revocation');
  ...
  const anchorKey = generateKeys().publicKey;
  ...
  // Case 1: anchor + known + unknown attestations, no revocation
  // expected: +3 (anchor) +1 (known) +0 (unknown) = 4
  return att.score(target, {
    knownProviders: [knownProvider],
    anchorPubkey: anchorKey,
    attestations: [mkAtt(anchorKey), mkAtt(knownProvider), mkAtt(unknownProvider)],
    revocations: []
  }).then(s1 => {
    check('anchor+provider+unknown scores 4',
      s1.score === TRUST_WEIGHTS.ANCHOR + TRUST_WEIGHTS.PROVIDER + TRUST_WEIGHTS.UNKNOWN);
    check('breakdown counts are correct',
      s1.breakdown.ANCHOR === 1 && s1.breakdown.PROVIDER === 1 && s1.breakdown.UNKNOWN === 1);

    // Case 2: same attestations + an anchor revocation → 4 + (-10) = -6
    return att.score(target, {
      knownProviders: [knownProvider],
      anchorPubkey: anchorKey,
      attestations: [mkAtt(anchorKey), mkAtt(knownProvider), mkAtt(unknownProvider)],
      revocations: [mkRev(anchorKey)]
    });
  }).then(s2 => {
    check('anchor revocation drops score to -6', s2.score === 4 + TRUST_WEIGHTS.REVOCATION);
...
async function runHttpOffline() {
  ...
  const sampleProviders = [
    { pubkey: 'a'.repeat(64), id: 'p1', publishedAt: new Date(), name: 'Provider A',
      country: 'TZ', direction: 'off-ramp', service_type: 'currency-exchange',
    ...
    { pubkey: 'b'.repeat(64), id: 'p2', publishedAt: new Date(), name: 'Provider B',
      country: 'KE', direction: 'on-ramp', service_type: 'remittance',
      rails: { in: ['lightning'], out: ['bank'] },
      currency: 'KES', endpoint: 'https://b.example', health: 'https://b.example/health',
      status: 'paused', status_reason: 'maintenance', kyc: 'full', protocols: [], metadata: {} }
  ];
  ...
  res = await fetch(base + '/v1/services?service_type=remittance');
  body = await res.json();
  check('service_type filter narrows to remittance services', body.count === 1 && body.services[0].provider.name === 'Provider B');

  res = await fetch(base + '/v1/services?kyc=full');
  body = await res.json();
  check('kyc filter applies client-side', body.count === 1 && body.services[0].kyc === 'full');

  res = await fetch(base + '/v1/services?status=paused');
  body = await res.json();
  check('status filter applies client-side', body.count === 1 && body.services[0].status === 'paused');
...
// ---------------------------------------------------------------------------
// Provider-hosted discovery — offline (loopback only)
// ---------------------------------------------------------------------------

async function runProviderHostedOffline() {
  console.log("\n=== Provider-hosted discovery (offline, loopback) ===");
  ...
  const app = createProviderHostedTransport({ document });
  const server = await listenEphemeral(app);
  const base = 'http://127.0.0.1:' + server.address().port;

  let res = await fetch(base + WELL_KNOWN_PATH);
  let body = await res.json();

  check('provider-hosted discovery endpoint returns 200', res.status === 200);
  check('provider-hosted response preserves protocol version', body.protocol === 'lipa' && body.version === '0.3');
  check('provider-hosted response preserves provider identity',
    body.provider.name === 'Example Provider' &&
    body.provider.pubkey === 'a'.repeat(64));
  check('provider-hosted response can contain multiple services',
    Array.isArray(body.services) && body.services.length === 2);
  check('provider-hosted response preserves service fields',
    body.services[0].direction === 'off-ramp' &&
    body.services[0].rails.out.includes('mtn-momo') &&
    body.services[0].currencies.includes('ZMW'));
  check('provider-hosted response preserves different service types',
    body.services[1].service_type === 'airtime-data');

  res = await fetch(base + '/not-found');
  body = await res.json();
  check('provider-hosted unknown route returns 404',
    res.status === 404 && body.error === 'not_found');

  res = await fetch(base + WELL_KNOWN_PATH, { method: 'OPTIONS' });
  check('provider-hosted endpoint supports CORS preflight',
  ...
  function assertThrows(fn, expected) {
    ...
    check(
      `Provider-hosted rejects invalid document (${expected})`,
      message !== null && message.includes(expected)
    );
  }
  assertThrows(() => createProviderHostedTransport({ document: null }), "document is required");
  assertThrows(() => createProviderHostedTransport({ document: { ...document, protocol: "invalid" } }), "protocol");
  assertThrows(() => createProviderHostedTransport({ document: { ...document, version: "" } }), "version");
  assertThrows(() => createProviderHostedTransport({ document: { ...document, provider: {} } }), "provider.name");
  assertThrows(() => createProviderHostedTransport({ document: { ...document, provider: { name: "Test", pubkey: "short" } } }), "pubkey");
  assertThrows(() => createProviderHostedTransport({ document: { ...document, services: [] } }), "services");
  assertThrows(() => createProviderHostedTransport({ document: { ...document, services: [{ id: "s1" }] } }), "countries");
  assertThrows(() => createProviderHostedTransport({ document: { ...document, services: [{ ...document.services[0], status: "paused", status_reason: "not_a_real_reason" }] } }), "status_reason");
  assertThrows(() => createProviderHostedTransport({ document: { ...document, services: [{ ...document.services[0], status: "active", status_reason: "maintenance" }] } }), "only valid when");

  let pausedThrew = false;
  try {
    createProviderHostedTransport({ document: { ...document, services: [{ ...document.services[0], status: "paused", status_reason: "out_of_float" }] } });
  } catch {
    pausedThrew = true;
  }
  check('provider-hosted accepts valid status_reason when status is paused', !pausedThrew);

  check('provider-hosted document validation enforces schema constraints', true);
  check('provider-hosted transport requires a document', threw);
}// ---------------------------------------------------------------------------
...
async function runHardening() {
  ...
  const baseListing = {
    id: 's1', name: 'P', country: 'TZ', direction: 'off-ramp', service_type: 'currency-exchange',
...
async function main() {
  ...
  await runProviderHostedOffline();
