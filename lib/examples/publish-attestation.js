 ...
 *
 * Usage:
 *   node examples/publish-attestation.js vouch <target_pubkey>
 *   node examples/publish-attestation.js revoke <target_pubkey> "reason"
 *   node examples/publish-attestation.js score <target_pubkey>
 *
...
async function main() {
  console.log('=== Lipa Bitcoin Service Discovery Protocol ===');
  ...
  try {
    if (action === 'vouch') {
      ...
      // In production, pass real knownProviders and the anchor pubkey.
      const result = await attestation.score(target, {
        knownProviders: [],
        anchorPubkey: process.env.ANCHOR_PUBKEY || null
      });
