...
const { KINDS, DEFAULT_RELAYS, DEFAULT_QUERY_LIMIT, TRUST_WEIGHTS, ALT_TEXT, PROTOCOL_VERSION } = require('./config');
...
class Attestation {
  ...
  buildVouchEvent(pubkey, options = {}) {
    ...
    if (!/^[0-9a-f]{64}$/.test(pubkey)) {
      throw new Error('pubkey must be a 64-char lowercase hex string (not npub)');
    }
    ...
    // Use the full pubkey so two targets can never collide on a shared prefix.
    const d = options.id || `vouch-${pubkey}`;

    const tags = [
      ['d', d],
      ['alt', ALT_TEXT[KINDS.ATTESTATION]],
      ['v', PROTOCOL_VERSION],
      ['p', pubkey],
  ...
  buildRevokeEvent(pubkey, reason, options = {}) {
    ...
    const d = options.id || `revoke-${pubkey}`;

    const tags = [
      ['d', d],
      ['alt', ALT_TEXT[KINDS.REVOCATION]],
      ['v', PROTOCOL_VERSION],
      ['p', pubkey],
   ...
   * @private
   */
  _tierOf(authorPubkey, knownSet, anchorPubkey) {
    if (anchorPubkey && authorPubkey === anchorPubkey) return 'ANCHOR';
   ...
   *   - Anchor attestation:              +3  (TRUST_WEIGHTS.ANCHOR)
   ...
   *
   * Only recognised keys (anchor or known providers) can move the score, in
   ...
   * @param {string} [options.anchorPubkey] - The designated trust anchor pubkey
  ...
  async score(pubkey, options = {}) {
    ...
    const anchorPubkey = options.anchorPubkey || null;
    ...
    const breakdown = { ANCHOR: 0, PROVIDER: 0, UNKNOWN: 0, REVOCATION: 0 };
    let score = 0;

    for (const a of attestations) {
      const tier = this._tierOf(a.attester, knownSet, anchorPubkey);
      breakdown[tier] += 1;
      score += TRUST_WEIGHTS[tier];
    }

    // Revocations only count from recognised keys (anchor or known providers).
    for (const r of revocations) {
      const tier = this._tierOf(r.revoker, knownSet, anchorPubkey);
      if (tier === 'ANCHOR' || tier === 'PROVIDER') {
