...
class Publisher {
  ...
  _validate(listing) {
    ...
    if (listing.rail_out !== undefined && listing.rail_out !== null) inSet('rail_out', VOCAB.rail_out);
    if (listing.service_type !== undefined && listing.service_type !== null) inSet('service_type', VOCAB.service_type);
   ...
   * @param {string} [listing.rail_out] - 'm-pesa' | 'mtn-momo' | 'airtel-money' | 'bank' | 'cash'. Required for 'currency-exchange' and 'remittance'; optional otherwise.
  ...
  buildEvent(listing) {
    ...
    // Validate required fields are present and are strings. rail_out is only
    // required for service types that genuinely deliver fiat through a rail
    // (currency-exchange, remittance) — airtime, bills, and merchant payments
    // may settle the obligation directly with no fiat rail involved.
    const required = ['id', 'name', 'country', 'direction', 'rail_in', 'currency', 'endpoint', 'health'];
    if (RAIL_OUT_REQUIRED_FOR.includes(serviceType)) {
      required.push('rail_out');
    }
    ...
    for (const [key, value] of Object.entries(optional)) {
      if (value !== undefined && value !== null) {
        const tagKey = key === 'rail_out' ? FILTER_TAGS.rail_out : key;
        tags.push([tagKey, String(value)]);
      }
    }
