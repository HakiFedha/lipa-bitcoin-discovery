...
function normalizeProvider(p) {
  return {
    ...
    // Canonical parsed shape is rails.in / rails.out. The scalar fallback
    // keeps older transport-specific parsed objects compatible.
    rails: {
      in: Array.isArray(p.rails?.in) ? p.rails.in : (Array.isArray(p.rail_in) ? p.rail_in : (p.rail_in ? [p.rail_in] : [])),
      out: Array.isArray(p.rails?.out) ? p.rails.out : (Array.isArray(p.rail_out) ? p.rail_out : (p.rail_out ? [p.rail_out] : []))
    },
