...
class Querier {
  ...
  _parseEvent(event) {
    ...
    for (const [key, value] of event.tags) {
      if (key === FILTER_TAGS.rail_in) {
        railIn.push(value);
        continue;
      }
      if (key === FILTER_TAGS.rail_out) {
        railOut.push(value);
        continue;
      }
    ...
    return {
      ...
      service_type: tags[FILTER_TAGS.service_type] || DEFAULT_SERVICE_TYPE,
      rail_in: railIn,
      rail_out: railOut,
      currency: tags[FILTER_TAGS.currency],
      endpoint: tags.endpoint || null,
      health: tags.health || null,
      status: tags.status,
      status_reason: tags.status_reason || null,
  ...
  _buildFilter(filters = {}) {
    ...
    if (filters.service_type) nostrFilter['#' + FILTER_TAGS.service_type] = [filters.service_type];
   ...
   * @param {string} [filters.rail_in] - Match a service with this inbound rail
   * @param {string} [filters.rail_out] - Match a service with this outbound rail
  ...
  async find(filters = {}) {
    ...
    const providers = events
      ...
      .filter(p => {
        ...
        if (filters.service_type && p.service_type !== filters.service_type) return false;
        if (filters.rail_in && !p.rail_in.includes(filters.rail_in)) return false;
  ...
  async checkHealth(healthUrl, timeout = 5000) {
    ...
    try {
      ...
      return {
        ...
        healthy: HEALTHY_STATUS_VALUES.includes(String(data.status || '').toLowerCase())
      };
    } catch (err) {
      return null;
    }
